...
export default {
    data(){
        return{
            skills: ["Vue, HTML, CSS, Javascript, PWA, SASS, Node.js"],
            colors: ["purple","white","orange", "FUCHSIA", "blue", "red", "green", "yellow"],
            skill: "",
            current: 0,
            counter: 0,
            up: true,
    ...
    methods: {
        ...
        addColor(text) {
            return text.split(",")
            .map((word, index) => `<span style="color:${this.colors[index]}">${word}</span>`)
            .join(",")
        }
    },
    mounted() {
